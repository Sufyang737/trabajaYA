...
export default function SaveProfileButton({ profileId, className, variant = "button" }: { profileId: string; className?: string; variant?: "button" | "icon" }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch(`/api/saves/profiles?profile_id=${encodeURIComponent(profileId)}`);
        if (!res.ok) {
          if (res.status === 404) setUnsupported(true);
          return;
        }
  ...
  async function toggle(e: React.MouseEvent) {
    ...
    try {
      setLoading(true);
      if (saved) {
        const res = await fetch(`/api/saves/profiles?profile_id=${encodeURIComponent(profileId)}`, { method: "DELETE" });
        if (!res.ok) throw new Error(await res.text());
        setSaved(false);
      } else {
        const res = await fetch(`/api/saves/profiles`, {
          method: "POST",
